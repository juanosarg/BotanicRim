...
namespace BotanicRim
{
    public class PipeNetGrid : MapComponent
    {
        ...
        private NutrientPipeNet[] netGrid;

        private Dictionary<NutrientPipeNet, List<IntVec3>> powerNetCells = new Dictionary<NutrientPipeNet, List<IntVec3>>();

        public PipeNetGrid(Map map) : base(map)
        {
            this.map = map;
            this.netGrid = new NutrientPipeNet[map.cellIndices.NumGridCells];

        }

     

        public NutrientPipeNet TransmittedPowerNetAt(IntVec3 c)
        {
